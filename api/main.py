...
def get_report(
    ...
    sku_scenario: str = Query("default", description="SKU 数据场景：default/class_demo"),
    ...
    category_mapping: str = Query(
        "",
        description="SKU→品类映射表绝对路径（.json/.csv）；空则不启用映射",
    ),
) -> Dict[str, Any]:
    if not as_of:
    ...
    if feedback_memory.strip():
        ...
        fb = p
    cm: Optional[Path] = None
    if category_mapping.strip():
        p = Path(category_mapping).expanduser()
        if not p.is_file():
            raise HTTPException(status_code=400, detail=f"找不到类目映射文件：{p}")
        cm = p
...
def get_report_markdown(
    ...
    as_of: str = Query(""),
    sku_scenario: str = Query("default"),
