...
import json
import json_repair
from typing import Any, AsyncIterator, overload, Literal, AbstractSet
...
async def rebuild_knowledge_from_chunks(
    ...
    skip_other_entities_in_graph = bool(
        (global_config.get("addon_params") or {}).get(
            "skip_other_entities_in_graph", False
        )
    )
    isolate_other_entity_edges_in_graph = bool(
        (global_config.get("addon_params") or {}).get(
            "isolate_other_entity_edges_in_graph", False
        )
    )
        ...
        try:
            ...
            for result in results:
                entities, relationships = await _rebuild_from_extraction_result(
                    ...
                    timestamp=result[1],
                    skip_other_entities_in_graph=skip_other_entities_in_graph,
                    isolate_other_entity_edges_in_graph=isolate_other_entity_edges_in_graph,
                )
...
async def _process_extraction_result(
    ...
    *,
    skip_other_entities_in_graph: bool = False,
    isolate_other_entity_edges_in_graph: bool = False,
    prior_other_entity_names_for_edge_filter: AbstractSet[str] | None = None,
    ...
    Args:
        ...
        skip_other_entities_in_graph: True 时不写入 other 节点，且丢弃任一端为「本结果中 entity 行标为 other 的名字」的关系边。
        isolate_other_entity_edges_in_graph: True 时 other 节点仍写入，但同样丢弃上述关系边（利于 other 度数为 0；跨 chunk 仍可能从别处分到边）。
        prior_other_entity_names_for_edge_filter: 与本轮 LLM 输出合并后再做关系边过滤（用于 glean：首轮已标为 other 的名字在次轮常不再重复出现在 entity 行）。
    Returns:
        tuple: (nodes_dict, edges_dict, other_names_in_this_parse) — 第三项为本轮 entity 行中标记为 other 的规范化名称，供上层传入下一轮 glean。
    """
    ...
    # 先扫一遍：收集本抽取结果里在 entity 行中被标为 other 的规范化名称（与主循环解析一致，避免 relation 行在 entity 行之前时漏标）
    names_marked_as_other: set[str] = set()
    for record in fixed_records:
        ...
        if entity_data is not None:
            ...
            if entity_data.get("entity_type", "").strip().lower() in ("other",):
                names_marked_as_other.add(truncated_name)

    edge_drop_for_relations: set[str] = set(names_marked_as_other)
    if prior_other_entity_names_for_edge_filter:
        edge_drop_for_relations.update(prior_other_entity_names_for_edge_filter)
    ...
    for i, record in enumerate(fixed_records, start=1):
        ...
        if entity_data is not None:
            ...
            if skip_other_entities_in_graph and entity_data.get(
                "entity_type", ""
            ).strip().lower() in ("other",):
                await _cooperative_yield(i, every=8)
                continue
        ...
        if relationship_data is not None:
            ...
            if (
                skip_other_entities_in_graph or isolate_other_entity_edges_in_graph
            ) and (
                truncated_source in edge_drop_for_relations
                or truncated_target in edge_drop_for_relations
            ):
                await _cooperative_yield(i, every=8)
                continue
    ...
    return dict(maybe_nodes), dict(maybe_edges), frozenset(names_marked_as_other)
...
async def extract_entities(
    ...
    async def _process_single_content(chunk_key_dp: tuple[str, TextChunkSchema]):
        ...
        _etype_defs = (global_config.get("addon_params") or {}).get(
            "entity_type_definitions"
        )
        if isinstance(_etype_defs, str) and _etype_defs.strip():
            entity_extraction_system_prompt = (
                entity_extraction_system_prompt
                + "\n\n--- 业务 entity_type 边界定义（必须严格遵守） ---\n"
                + _etype_defs.strip()
            )
        ...
        (
            maybe_nodes,
            maybe_edges,
            initial_other_names_for_edge_filter,
        ...
        if entity_extract_max_gleaning > 0:
            ...
            if token_count > max_input_tokens:
                ...
                glean_nodes, glean_edges, _ = await _process_extraction_result(
                    ...
                    skip_other_entities_in_graph=skip_other_entities_in_graph,
                    isolate_other_entity_edges_in_graph=isolate_other_entity_edges_in_graph,
                    prior_other_entity_names_for_edge_filter=initial_other_names_for_edge_filter,
                )
